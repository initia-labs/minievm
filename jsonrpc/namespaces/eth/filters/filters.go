...
func NewBlockFilter(logger log.Logger, backend backend.JSONRPCBackend, criteria filters.FilterCriteria) *Filter {
...
func newFilter(logger log.Logger, backend backend.JSONRPCBackend, criteria filters.FilterCriteria, bloomFilters [][]BloomIV) *Filter {
	return &Filter{
		logger:       logger,
		backend:      backend,
		criteria:     criteria,
...
func (f *Filter) Logs(_ context.Context, logLimit int, blockLimit int64) ([]*ethtypes.Log, error) {
	...
	if f.criteria.BlockHash != nil && *f.criteria.BlockHash != (common.Hash{}) {
		...
		blockRes, err := f.backend.GetBlockByHash(*f.criteria.BlockHash, false)
		if err != nil {
			return nil, fmt.Errorf("failed to fetch block result by hash %s: %w", f.criteria.BlockHash, err)
		}

		bloom, exist := blockRes["logsBloom"].(ethtypes.Bloom)
		if !exist {
			return nil, fmt.Errorf("failed to fetch header")
		}

		blockNum := (blockRes["number"].(*hexutil.Big)).ToInt().Int64()

		return f.blockLogs(blockNum, bloom)
	}
	...
	for height := from; height <= to; height++ {
		blockHdr, err := f.backend.GetHeaderByNumber(rpc.BlockNumber(height))
		if err != nil {
			f.logger.Debug("failed to fetch block header", "height", height, "error", err.Error())
			return nil, nil
		}

		bloom := blockHdr.Bloom
...
func (f *Filter) blockLogs(blockHeight int64, bloom ethtypes.Bloom) ([]*ethtypes.Log, error) {

...
func Test_SnapshotRevert(t *testing.T) {
	...
	_, evm, _, err := input.EVMKeeper.CreateEVM(sdkCtx, evmtypes.StdAddress)
	require.NoError(t, err)
...
func Test_Selfdestruct6780_InSameTx(t *testing.T) {
	...
	caller := common.BytesToAddress(addr.Bytes())
	_, evm, _, err := input.EVMKeeper.CreateEVM(sdkCtx, caller)
	require.NoError(t, err)
	stateDB := evm.StateDB.(*state.StateDB)
...
func Test_Empty(t *testing.T) {
	...
	// get state db
	_, evm, _, err := input.EVMKeeper.CreateEVM(ctx, evmtypes.StdAddress)
	require.NoError(t, err)

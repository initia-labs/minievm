...
var _ cosmoskeeper.ViewKeeper = (*EVMViewKeeper)(nil)

// EVMViewKeeper implements a read only keeper implementation of ViewKeeper.
...
// NewEVMViewKeeper returns a new EVMViewKeeper.
func NewEVMViewKeeper(
	cdc codec.BinaryCodec,
	storeService store.KVStoreService,
	ak types.AccountKeeper,
	ek evmtypes.IERC20Keeper,
...
func (k EVMViewKeeper) HasBalance(ctx context.Context, addr sdk.AccAddress, amt sdk.Coin) bool {
	return k.GetBalance(ctx, addr, amt.Denom).IsGTE(amt)
}

// GetAllBalances returns all the account balances for the given account address.
func (k EVMViewKeeper) GetAllBalances(ctx context.Context, addr sdk.AccAddress) sdk.Coins {
...
func (k EVMViewKeeper) GetAccountsBalances(ctx context.Context) []types.Balance {
...
// by address.
func (k EVMViewKeeper) GetBalance(ctx context.Context, addr sdk.AccAddress, denom string) sdk.Coin {
	balance, err := k.ek.GetBalance(ctx, addr, denom)
	if err != nil {
		panic(err)
	}
...
func (k EVMViewKeeper) IterateAccountBalances(ctx context.Context, addr sdk.AccAddress, cb func(sdk.Coin) bool) {
	err := k.ek.IterateAccountBalances(ctx, addr, func(c sdk.Coin) (bool, error) {
		return cb(c), nil
	})
	if err != nil {
		panic(err)
...
// callback, iteration is halted.
func (k EVMViewKeeper) IterateAllBalances(ctx context.Context, cb func(sdk.AccAddress, sdk.Coin) bool) {
	k.ak.IterateAccounts(ctx, func(account sdk.AccountI) bool {
		addr := account.GetAddress()
		err := k.ek.IterateAccountBalances(ctx, addr, func(coin sdk.Coin) (bool, error) {
...
func (k EVMViewKeeper) LockedCoins(ctx context.Context, addr sdk.AccAddress) sdk.Coins {
	return sdk.NewCoins()
}
...
// returned.
func (k EVMViewKeeper) SpendableCoins(ctx context.Context, addr sdk.AccAddress) sdk.Coins {
...
// is returned.
func (k EVMViewKeeper) SpendableCoin(ctx context.Context, addr sdk.AccAddress, denom string) sdk.Coin {
...
func (k EVMViewKeeper) spendableCoins(ctx context.Context, addr sdk.AccAddress) (spendable, total sdk.Coins) {
...
func (k EVMViewKeeper) ValidateBalance(ctx context.Context, addr sdk.AccAddress) error {

...
// CollectResultsEth uses eth_getTransactionReceipt to match JSON-RPC submitted txs
func CollectResultsEth(
	ctx context.Context,
	cl *cluster.Cluster,
	cfg BenchConfig,
	load LoadResult,
	...
	// first we collect block stats (same as CollectResults)
	var blockStats []BlockStat
	blockTimeByHeight := make(map[int64]time.Time)
	...
	for h := startHeight; h <= endHeight; h++ {
		...
		blockStats = append(blockStats, bs)
		blockTimeByHeight[h] = block.BlockTime
	}

	// query receipts for each submitted tx to determine inclusion
	var (
		latencies []float64
		included  int
	)
	for _, sub := range load.Submissions {
		...
		receipt, err := cl.EthGetTransactionReceipt(ctx, 0, sub.TxHash)
		...
		if blockNumHex != "" {
			var blockNum int64
			fmt.Sscanf(strings.TrimPrefix(blockNumHex, "0x"), "%x", &blockNum)
			if bt, ok := blockTimeByHeight[blockNum]; ok {
				latencyMs := float64(bt.Sub(sub.SubmitTime).Milliseconds())
				if latencyMs < 0 {
					latencyMs = 0
				}
				latencies = append(latencies, latencyMs)

...
func (im IBCMiddleware) OnRecvPacket(
	...
	// Validate the memo
	isEVMRouted, msg, err := validateAndParseMemo(data.GetMemo(), data.Receiver)
	if !isEVMRouted {
		return im.app.OnRecvPacket(ctx, packet, relayer)
	} else if err != nil {
...
func (im IBCMiddleware) execMsg(ctx sdk.Context, msg *evmtypes.MsgCall) (*evmtypes.MsgCallResponse, error) {
	moveMsgServer := evmkeeper.NewMsgServerImpl(im.evmKeeper)
	return moveMsgServer.Call(ctx, msg)
}
